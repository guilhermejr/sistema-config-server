...
The **Config Server**. Serves every microservice's properties from a Git repository, so the services carry almost no configuration of their own.
...
Configuration lives in a separate repo, one folder per application:
...
Secrets are **not** in that repo. The files carry placeholders (`${autenticacaoDBPass}`) that each service resolves against **Vault** on its own.

This server reads its own configuration from Vault only — it does not consume the shared `application.yml` it serves. So settings placed there (`show-details`, exposure) apply to the clients, not to this process.

## Startup order

Depends on Vault and on access to the config Git repository. Must be up **before** the services that read from it.

## Security

HTTP Basic, credentials from `configServerUser` / `configServerPass` in Vault. `/actuator/health` is public; the rest of `/actuator/**` needs `ROLE_ACTUATOR`.
